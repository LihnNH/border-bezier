(() => {
  ...
  function browserMetrics(element, width, height) {
    ...
    const document2 = element?.ownerDocument ?? globalThis.document;
    const getStyles = view?.getComputedStyle?.bind(view);
    const rootFontSize = getStyles && document2?.documentElement ? finiteOr(getStyles(document2.documentElement).fontSize, 16) : 16;
  ...
  function observeBorderBezier(root = globalThis.document, options = {}) {
    ...
    const view = document2?.defaultView ?? globalThis;
  ...
  // src/global.js
  globalThis.BorderBezier = index_default;
